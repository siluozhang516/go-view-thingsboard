...
import tinigsboard, { EntitySubscriptionService, FormValue, PublicParams } from '@thingsboard/core'
...
export const useChartDataFetch = (
  ...
  const requestIntervalFn = () => {
    ...
    if (requestDataType.value === RequestDataTypeEnum.Thingsboard) {
      if (sub7$) sub7$.unsubscribe()
      if( !subscribe.value.formValue) return
      let params: PublicParams = {
        ...subscribe.value.datakeys,
        pageLink: subscribe.value.pageLink,
        keyFilters: subscribe.value.keyFilter || [],
        isPage: subscribe.value.timeWindowConfig ? false : true,
        widgetType: subscribe.value.timeWindowConfig ? tinigsboard.widgetType.timeseries : tinigsboard.widgetType.latest
      }
      if (subscribe.value.timeWindowConfig) {
        params.timeWindowConfig = subscribe.value.timeWindowConfig as tinigsboard.Timewindow
      }

      sub7$ = EntitySubscriptionService.subscribeEntityInfoPublic(
        subscribe.value.formValue as FormValue,
        params
      ).subscribe(
        (res: any) => {
          try {
