...
const configTsModules: Record<string, { default: string }> = import.meta.glob('./components/**/config.ts', {
  eager: true
})
...
const loadConfig = (packageName: string, categoryName: string, keyName: string) => {
  ...
  if (!componentCacheMap.has(key)) {
    componentCacheMap.set(key, import(findPathByConfigTs(keyName)))
  }
 ...
 * 匹配config.ts的路径
 */
const findPathByConfigTs = (chartName:string)=>{
  for (const key in configTsModules) {
...
export const createComponent = async (targetData: ConfigType) => {
  ...
  const config = new chart.default()
  // meta2d图根据dataset的值去加载对应的json
  if(config.key === PackagesCategoryEnum.META2D) {
    config.chartConfig.dataset = targetData.dataset
  }
  return config
}
