<template>
  <div class="go-content-charts-item-animation-patch">
    <div ref="contentChartsItemBoxRef" class="go-content-charts-item-box"
      :class="[chartMode === ChartModeEnum.DOUBLE ? 'double' : 'single']">
      <!-- 每一项组件的渲染 -->
      <div class="item-box" v-for="(item, index) in menuOptions" :key="item.title" draggable
        @dragstart="!item.disabled && dragStartHandle($event, item)" @dragend="!item.disabled && dragendHandle()"
        @dblclick="dblclickHandle(item)" @click="clickHandle(item)">
...
import { PropType, watch, ref, Ref, computed, nextTick, unref } from 'vue'
...
const componentList = computed(() => chartEditStore.getComponentList)
 ...
 * 从图形编辑查找meta2d
 */
const matchMeta = (item: ConfigType) => {
  if(item.dataset){
    const meta = unref(componentList).find(k => k.chartConfig.dataset === item.dataset)
    if (meta && item.key === PackagesCategoryEnum.META2D) return true
  }
