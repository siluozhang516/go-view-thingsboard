    ...
    <n-layout-sider collapse-mode="transform" :collapsed-width="0" :width="350" :collapsed="collapsed"
      ...
      <content-box class="go-content-configurations go-boderbox" :show-top="false" :depth="2">
        ...
        <n-tabs v-if="selectTarget" v-model:value="tabsSelect" class="tabs-box" size="small" type="segment">
          <n-tab-pane v-for="item in tabsList" :key="item.key" :name="item.key" size="small"
            display-directive="show:lazy">
            <template #tab>
              <n-space>
                <span>{{ item.title }}</span>
...
import { ref, toRefs, watch, computed, unref } from 'vue'
...
const tabsList = computed(() => {
  if (selectTarget.value) {
    if (unref(selectTarget)!.isGroup) {
      return chartsDefaultTabList
    }
    if (unref(selectTarget)!.key === PackagesCategoryEnum.META2D) {
      return metaTabList
    }
    return chartsTabList
