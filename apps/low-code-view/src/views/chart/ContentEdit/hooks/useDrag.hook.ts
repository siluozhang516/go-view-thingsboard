...
// * 移动锚点
export const useMousePointHandle = (e: MouseEvent, point: string, attr: PickCreateComponentType<'attr'>, item) => {
  e.stopPropagation()
  e.preventDefault()
  ...
  const mouseup = () => {
    ...
    document.removeEventListener('mouseup', mouseup)
    // 更新meta2d画布尺寸
    if (item.key === PackagesCategoryEnum.META2D) {
      meta2d.resize( attr.w, attr.h)
      meta2d.fitView()
    }
  }
